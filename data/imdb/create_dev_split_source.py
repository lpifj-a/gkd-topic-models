...
if __name__ == "__main__":
    outdir = Path("processed_source-dev")
    outdir.mkdir(exist_ok=True)

    # copy over the train files
    shutil.copy("train.jsonlist", Path(outdir, "train.jsonlist"))
    shutil.copy("processed_source/train.npz", Path(outdir, "train.npz"))
    shutil.copy("processed_source/train.ids.json", Path(outdir, "train.ids.json"))
    shutil.copy("processed_source/train.vocab.json", Path(outdir, "train.vocab.json"))

    # read in test
    test_jsonlist = utils.load_jsonlist("test.jsonlist")
    test_counts = utils.load_sparse("processed_source/test.npz")
    test_ids = utils.load_json("processed_source/test.ids.json")
