...
class Scholar(object):
    def __init__(
        ...
        config,
        trial, #optuna
        doc_reconstruction_weight_list, # optuna
        doc_reconstruction_temp_list, # optuna
        RCD_weight, # optuna
        FKD_weight, # optuna
        FKD_temp, # optuna
        fkd_output_emb_dim, # optuna
        alpha=1.0,
        learning_rate=0.001,
    ...
    def fit(
        ...
        if rcd_teacher_emb is not None:
            rcd_teacher_emb = torch.Tensor(rcd_teacher_emb).to(self.device)
        if rcd_teacher_emb_contrast is not None:
            rcd_teacher_emb_contrast = torch.Tensor(rcd_teacher_emb_contrast).to(self.device)
        if fkd_teacher_emb is not None:
            fkd_teacher_emb = torch.Tensor(fkd_teacher_emb).to(self.device)
        ...
        z, eta, thetas, X_recon, Y_probs, losses = self._model(
            ...
            rcd_teacher_emb_contrast=rcd_teacher_emb_contrast,
...
class torchScholar(nn.Module):
    def __init__(
        ...
        self.use_pseudo_doc_layer = config["use_pseudo_doc_layer"]
        self.doc_reconstruction_weight_list = doc_reconstruction_weight_list
        self.doc_reconstruction_temp_list = doc_reconstruction_temp_list
        ...
        self.rcd_teacher_emb_dim = config["rcd_teacher_emb_dim"]
        self.rcd_n_data = config["rcd_n_data"]
        self.fkd_teacher_emb_dim = config["fkd_teacher_emb_dim"]
        self.fkd_output_emb_dim = fkd_output_emb_dim
        self.fkd_n_data = config["fkd_n_data"]
        self.RCD_weight = RCD_weight
        self.feature_based_KD = config["feature_based_KD"]
        self.fkd_loss_function = config["fkd_loss_function"]
        self.FKD_temp = FKD_temp
        self.FKD_weight = FKD_weight
        self.RCD = config["RCD"]
        ...
        if self.teacher_eta_dim_list:
            for eta_dim in self.teacher_eta_dim_list:
                if self.use_pseudo_doc_layer:
                    emb_size += self.words_emb_dim
                    self.pseudo_doc_layer = nn.Linear(
                        eta_dim, self.words_emb_dim
                    ).to(self.device)
                else:
                    emb_size += eta_dim
        if self.n_labels > 0:
        ...
        if self.RCD:
            ...
            self.linear_t = nn.Linear(self.rcd_teacher_emb_dim, 64) # 1024, 64
            # (i) anchor: teacher
        ...
        if self.feature_based_KD and self.fkd_output_emb_dim is not None:
            self.fkd_linear_s = nn.Linear(self.n_topics, self.fkd_output_emb_dim)
            self.fkd_linear_t = nn.Linear(self.fkd_teacher_emb_dim, self.fkd_output_emb_dim)
    ...
    def forward(
        ...
        if TE_list:
            if self.use_pseudo_doc:
                ...
                for (TE, t) in zip(TE_list, t_list):
                    ...
                    if self.use_pseudo_doc_layer:
                        pseudo_doc = F.softplus(self.pseudo_doc_layer(pseudo_doc))
        ...
        # Contrastive learning
        if self.RCD and rcd_teacher_emb is not None:
            ...
            # L2 normalization
            rcd_teacher_emb_l2 = F.normalize(rcd_teacher_emb, p=2, dim=1) 
            rcd_teacher_emb_contrast = F.normalize(rcd_teacher_emb_contrast, p=2, dim=1) # weight_v2 (batch_size*501, 1024)
            z_do_l2 = F.normalize(z_do, p=2, dim=1) 

            # linear transformation that can solve the dimension mismatch probrem
            f_t = self.linear_t(rcd_teacher_emb_l2) # anchor_v2  (batch_size, 64)
            f_s = self.linear_s(z_do_l2) #  batch_size, 64
            f_t_contrast = self.linear_t(rcd_teacher_emb_contrast) # batch_size*501, 64
        ...
        if self.feature_based_KD and fkd_teacher_emb is not None:
            fkd_student_z = z_do
            # Use transform matrix 
            if self.fkd_output_emb_dim is not None:
                if self.fkd_output_emb_dim == self.n_topics and self.fkd_teacher_emb_dim != self.n_topics:
                    fkd_student_output_emb = fkd_student_z # identity matrix
                    fkd_teacher_output_emb = self.fkd_linear_t(fkd_teacher_emb)
                else:
                    fkd_student_output_emb = self.fkd_linear_s(fkd_student_z)
                    fkd_teacher_output_emb = self.fkd_linear_t(fkd_teacher_emb)
            # No transformation matrices for both students and teachers (identity matrices)
            else:
                fkd_student_output_emb = fkd_student_z
                fkd_teacher_output_emb = fkd_teacher_emb
        else:
            fkd_student_output_emb = None
            fkd_teacher_output_emb = None
        ...
        if compute_loss:
            return (
                ...
                self._loss(
                    ...
                    out_1,
                    out_2,
                    fkd_teacher_output_emb,
                    fkd_student_output_emb
                ),
            )
        else:
    ...
    def _loss(
        ...
        if fkd_teacher_output_emb is not None:
            FKD_t = self.FKD_temp
            student_phi = F.softmax(fkd_student_output_emb / FKD_t, dim=1)
            teacher_phi = F.softmax(fkd_teacher_output_emb / FKD_t, dim=1)

            if self.fkd_loss_function == "MSE":
                mse = nn.MSELoss()
                fkd_loss = self.FKD_weight * mse(student_phi,teacher_phi)
            elif self.fkd_loss_function == "CE":
                fkd_loss = (self.FKD_weight * FKD_t * FKD_t) * -(teacher_phi* (student_phi + 1e-10).log()).sum(1)
                fkd_loss = fkd_loss.mean()
