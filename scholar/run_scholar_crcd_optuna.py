...
from scholar_crcd_optuna import Scholar
...
def main(call=None):
    ...
    parser.add_argument(
        "--use-pseudo-doc-layer",
        action="store_true",
        default=False,
        help="Use a pseudo document projection layer",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--rcd-teacher-embedding-dir",
        dest="rcd_teacher_embedding",
        type=str,
        default=None,
        help="Teacher embedding file for relation based KD",
    )
    parser.add_argument(
        "--fkd-teacher-embedding-dir",
        dest="fkd_teacher_embedding",
        type=str,
        default=None,
        help="Teacher embedding file for feature based KD",
    )
    # parser.add_argument(
    #     "--RCD-weight",
    #     type=float,
    ...
    parser.add_argument(
        ...
        dest="ResKD",
        default=False,
        help="Response based Knowledge Distillation",
    )
    parser.add_argument(
        "--feature-based-KD",
        action="store_true",
    ...
    parser.add_argument(
        "--FKD-loss-function",
        dest="fkd_loss_function",
        default="MSE",
        type=str,
        help="Feature based Knowledge Distillation loss function. MSE or CE",
    )
    # parser.add_argument(
    #     "--FKD-temp",
    #     dest="FKD_temp",
    ...
    # load teacher_embedding for relation based knowledge distillation
    if options.rcd_teacher_embedding is not None:
        rcd_teacher_emb = np.load(options.rcd_teacher_embedding)
        print("RCD teacher emb size:",rcd_teacher_emb.shape)
        rcd_teacher_emb_dim = rcd_teacher_emb.shape[1]
        rcd_n_data = rcd_teacher_emb.shape[0]

        nan_count = 0
        for emb in rcd_teacher_emb:
            if any(np.isnan(emb)):
                nan_count+=1

        isnan = np.isnan(rcd_teacher_emb).flatten()
        if any(isnan):
            print("There are NaN in teacher embeddings. Replacing NaN with mean.")
            print("A number of NaN:",nan_count)
            rcd_teacher_emb = np.nan_to_num(rcd_teacher_emb, nan=np.nanmean(rcd_teacher_emb))
    else:
        rcd_teacher_emb = None
        rcd_teacher_emb_dim = None
        rcd_n_data = None

    # load teacher_embedding for for feature based knowledge distillation
    if options.fkd_teacher_embedding is not None:
        fkd_teacher_emb = np.load(options.fkd_teacher_embedding)
        print("FKD teacher emb size:",fkd_teacher_emb.shape)
        fkd_teacher_emb_dim = fkd_teacher_emb.shape[1]
        fkd_n_data = fkd_teacher_emb.shape[0]

        nan_count = 0
        for emb in fkd_teacher_emb:
            if any(np.isnan(emb)):
                nan_count+=1

        isnan = np.isnan(fkd_teacher_emb).flatten()
        if any(isnan):
            print("There are NaN in teacher embeddings. Replacing NaN with mean.")
            print("A number of NaN:",nan_count)
            fkd_teacher_emb = np.nan_to_num(fkd_teacher_emb, nan=np.nanmean(fkd_teacher_emb))
    else:
        fkd_teacher_emb = None
        fkd_teacher_emb_dim = None
        fkd_n_data = None
    ...
    network_architecture = make_network(
        ...
        rcd_teacher_emb_dim=rcd_teacher_emb_dim,
        rcd_n_data = rcd_n_data,
        fkd_teacher_emb_dim=fkd_teacher_emb_dim,
        fkd_n_data = fkd_n_data
    )
    ...
    if options.epochs > 0:
        ...
        study.optimize(lambda trial : objective(
                                            ...
                                            rcd_teacher_emb=rcd_teacher_emb,
                                            fkd_teacher_emb=fkd_teacher_emb,
    ...
    if options.restart:
        ...
        if options.ResKD:
            ...
            for i in range(num_teachers):
                # doc_reconstruction_weight_list.append(trial.suggest_uniform("ResKD_weight"+str(i),0.1,0.99))
                # doc_reconstruction_temp_list.append(trial.suggest_uniform("softmax_temp"+str(i),1,3))
                doc_reconstruction_weight_list.append(trial.suggest_float("ResKD_weight",0.1,0.99))
                doc_reconstruction_temp_list.append(trial.suggest_float("softmax_temp",1,3))
        else:
            doc_reconstruction_weight_list = None
            doc_reconstruction_temp_list = None
        if options.RCD:
            RCD_weight = trial.suggest_float("RCD_weight",1,100)
        else:
            RCD_weight = 0
        if options.feature_based_KD:
            FKD_weight = trial.suggest_float("FeaKD_weight",1,100)
            FKD_temp = trial.suggest_float("FeaKD_temp",1,3)
            fkd_output_emb_dim = trial.suggest_int("FeaKD_output_dim",50,500)
        else:
            FKD_weight = 0
            FKD_temp = 1
            fkd_output_emb_dim = None
        ...
        model = Scholar(
            ...
            RCD_weight = RCD_weight, # optuna
            FKD_weight = FKD_weight, # optuna
            FKD_temp = FKD_temp, # optuna
            fkd_output_emb_dim = fkd_output_emb_dim, # optuna
            alpha=options.alpha,
...
def load_word_counts(input_dir, input_prefix, vocab=None):
    ...
    # positive index for contrastive learning
    # if input_prefix == "train":
    #     random.seed(10)
    #     positive_ids =  random.sample(ids,int(len(ids)/100))
    #     print("Positive index:",len(positive_ids))
    #     return X, vocab, row_selector, ids, positive_ids
...
def make_network(
    ...
    network_architecture = dict(
        ...
        use_pseudo_doc_layer=options.use_pseudo_doc_layer,
        # Use optuna
        ...
        fkd_loss_function = options.fkd_loss_function,
        # Use optuna
...
def train(
    ...
    mb_gen = create_minibatch(X, Y, PC, TC, DR, TE_list, rcd_teacher_emb=rcd_teacher_emb,fkd_teacher_emb=fkd_teacher_emb, batch_size=batch_size, rng=rng)
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):   
            # get a minibatch
            batch_xs, batch_ys, batch_pcs, batch_tcs, batch_drs, batch_tes, batch_rcd_teacher_emb, batch_rcd_teacher_emb_contrast, batch_fkd_teacher_emb = next(mb_gen)
            ...
            z, eta, cost, recon_y, thetas, nl, kld, reskd, fkd, contrast = model.fit(
                ...
                rcd_teacher_emb=batch_rcd_teacher_emb,
                rcd_teacher_emb_contrast=batch_rcd_teacher_emb_contrast,
                fkd_teacher_emb=batch_fkd_teacher_emb,
            )
    ...
    # 早期終了せずにモデルの学習を終えた場合，トピックごとのNPMIを保存する
    print("Saving NPMI for each topic")
    print(best_npmi_for_each_topic)
    os.makedirs(options.output_dir+"/npmi_for_each_topic", exist_ok=True)
    dt_now = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S{}').format('')
...
def create_minibatch(X, Y, PC, TC, DR, TE_list, rcd_teacher_emb=None, fkd_teacher_emb=None, batch_size=200, rng=None):
    # Yield a random minibatch
    while True:
        # Return random data samples of a size 'minibatch_size' at each iteration
        if rng is not None:
            ixs = rng.randint(X.shape[0], size=batch_size) # positive
            # negative index
            index = np.arange(X.shape[0])
            neg = np.delete(index,ixs)
            negative_idx = rng.choice(neg, 500)
        else:
            ixs = np.random.randint(X.shape[0], size=batch_size) # positive
            # negative index
            index = np.arange(X.shape[0])
            neg = np.delete(index,ixs)
        ...
        if rcd_teacher_emb is not None:
            rcd_teacher_emb_mb = rcd_teacher_emb[ixs, :].astype("float32")
            rcd_teacher_emb_contrast_mb = rcd_teacher_emb[contrast_idx, :].astype("float32")
        else:
            rcd_teacher_emb_mb=None
            rcd_teacher_emb_contrast_mb=None

        if fkd_teacher_emb is not None:
            fkd_teacher_emb_mb = fkd_teacher_emb[ixs, :].astype("float32")
        else:
            fkd_teacher_emb_mb=None


        yield X_mb, Y_mb, PC_mb, TC_mb, DR_mb, TE_mb_list, rcd_teacher_emb_mb, rcd_teacher_emb_contrast_mb, fkd_teacher_emb_mb
